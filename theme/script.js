...
Dropdown.prototype = {
  ...
  focusByChar: function(currentItem, char) {
    ...
    const itemChars = this.menuItems.map(
      (menuItem) => menuItem.textContent.trim()[0].toLowerCase()
    );
  ...
  menuKeyHandler: function(e) {
    ...
    switch (key2) {
      ...
      default: {
        if (isPrintableChar(key2)) {
          e.stopPropagation();
          e.preventDefault();
          this.focusByChar(currentElement, key2);
        }
      }
    }
  }
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const searchInputs = searchForms.map(
    (form) => form.querySelector("input[type='search']")
  );
